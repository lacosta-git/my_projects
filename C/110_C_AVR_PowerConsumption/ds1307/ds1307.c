...
/*
 * initialize the Real Clock DS1307
 */
void ds1307_init() {
	#if DS1307_I2CINIT == 1
	//init i2c
 ...
 * set date
 */
uint8_t ds1307_setdate_BCD(uint8_t year, uint8_t month, uint8_t day,
 ...
 * get date
 */
void ds1307_getdate_BCD(uint8_t *year, uint8_t *month, uint8_t *day, uint8_t *hour, uint8_t *minute, uint8_t *second) {
	i2c_start_wait(DS1307_ADDR | I2C_WRITE);
 ...
 * Read RAM byte
 */
uint8_t ds1307_read_RAM_B(uint8_t address) {
	uint8_t data = 0;
	i2c_start_wait(DS1307_ADDR | I2C_WRITE);
	i2c_write(address); // Address pointer
	i2c_stop();
 ...
 * Read RAM word
 */
uint16_t ds1307_read_RAM_W(uint8_t address) {
	uint16_t data = 0;
	i2c_start_wait(DS1307_ADDR | I2C_WRITE);
	i2c_write(address); // Address pointer
	i2c_stop();
 ...
 * Write RAM byte
 */
void ds1307_write_RAM_B(uint8_t data, uint8_t address) {
	//write date
	i2c_start_wait(DS1307_ADDR | I2C_WRITE);
	i2c_write(address); // Address byte
	i2c_write(data);
	i2c_stop();
}
 ...
 * Write RAM word
 */
void ds1307_write_RAM_W(uint16_t data, uint8_t address) {
	uint8_t b_data = 0;
	//write date
	i2c_start_wait(DS1307_ADDR | I2C_WRITE);
	i2c_write(address); // Address byte
	b_data = (uint8_t)(data >> 8);
	i2c_write(b_data);
	b_data = (uint8_t)(data);
	i2c_write(b_data);
	i2c_stop();
