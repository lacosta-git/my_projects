...
void socketTCPStateMachine(uint8_t);
// Write
void wizzWrite(uint16_t, uint8_t);
// Read
unsigned char wizzRead(uint16_t);

#define TIME_TO_WAIT		500	// Variable uint16_t   Time for W5100 to perform action

// Parametry
#define KUCHNIA				0
#define SALON_JADALNIA		1
#define SALON_TARAS			2
#define POKOJ1_JULEK		3
#define POKOJ2_JAS			4
#define LAZIENKA_MALA		5
#define LAZIENKA_DUZA		6
#define POKOJ3_SYPIALNIA	7
#define MAX_NAME_LENGHT		16

// 23.6  = 236   wszystko x10
typedef struct {
	volatile int podlogaZadana;
	volatile int podlogaAktualna;
	volatile int podlogaWsKorekcji;
	volatile int podlogaKorekta;
	volatile int powietrzeAktualna;
	volatile int otwarcieZaworu;
} PARAMETRY;

typedef struct {
	uint16_t podlogaZadana;
	uint16_t podlogaWsKorekcji;
} PARAMETRY_E;

// web parameters
void initializeParametry(void);

#define EEPROM_INITIALIZED		0x54