...
#define S0_DIPR	   0x040C      // Socket 0: Destination IP Address Register: 0x040C–0x040F
#define S0_PROTO   0x0414	   // Socket 0: IP protocol
...
#define S0_RX_RSR  0x0426      // Socket 0: Rx Received Size Register: 0x0426 to 0x0427
...
#define TX_BUF_2K_MASK   0x07FF   // Tx 2K Buffer Mask:
#define RX_BUF_2K_MASK   0x07FF   // Rx 2K Buffer Mask:
#define NET_2K_MEMALLOC  0x55     // Use 2K of Tx/Rx Buffer

#define TX_BUF_MASK      0x1FFF   // Tx 8K Buffer Mask:
#define RX_BUF_MASK      0x1FFF   // Rx 8K Buffer Mask:
#define NET_MEMALLOC     0x0F     // Use 8K of Tx/Rx Buffer

// IR values
#define IR_SEND_OK		0x10	  // Data sent OK
#define IR_TIMEOUT		0x08	  // Timeout during connection establishment
#define IR_RECV			0x04	  // Data received
#define IR_DISCON		0x02	  // Connection termination is requested
...
uint16_t wizz_icmp_echo_msg(uint8_t*, uint8_t, uint16_t, uint16_t, uint8_t);
void wizz_send_msg(uint16_t, uint8_t *);
void wizzWrite(uint16_t, uint8_t);
unsigned char wizzRead(uint16_t);
void wizz_memcpy_wizz_to_uP(uint16_t, char *, uint16_t);
void wizz_read_message(uint16_t);
void wizz_read_socket_0_handler(void);
uint8_t wizz_write_socket_0_handler(uint8_t);
